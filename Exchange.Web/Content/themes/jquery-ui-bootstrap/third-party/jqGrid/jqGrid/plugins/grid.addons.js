...
// This is the ols search Filter method used in navigator.
                    ...
                    if ($("#" + fid).html() !== null) {
                        ...
                        $.each(colModel, function(i, v) {
                            var searchable = (typeof v.search === 'undefined') ? true : v.search,
                                ...
                                soptions = $.extend({}, { text: colNames[i], itemval: v.index || v.name }, searchoptions),
                    ...
                    $(data).each(function(j) {
                        ...
                        if (ind) {
                            ...
                            $(t.p.colModel).each(function(i) {
                                nm = jsonreader === true ? jsonmap || name : name;
        ...
        filterGrid: function(gridid, p) {
            ...
            return each(function() {
                var self = this;
                p = p;
                if (p.filterModel.length === 0 && p.gridModel === false) {
                ...
                p.gridid = gridid.indexOf("#") != -1 ? gridid : "#" + gridid;
                var gcolMod = $(p.gridid).jqGrid("getGridParam", 'colModel');
                if (gcolMod) {
                    if (p.gridModel === true) {
                        var thegrid = $(p.gridid)[0];
                        var sh;
                        ...
                        $.each(gcolMod, function(i, n) {
                            ...
                            if (editrules && editrules.searchhidden === true) {
                                sh = true;
                            } else {
                                if (hidden === true) {
                                    sh = false;
                            ...
                            if (search === true && sh === true) {
                                ...
                                tmpFil.name = name;
                                tmpFil.index = index || name;
                                // we support only text and selects, so all other to text
                                tmpFil.stype = edittype || 'text';
                                if (tmpFil.stype != 'select') {
                                    tmpFil.stype = 'text';
                                }
                                tmpFil.defval = defval || '';
                                tmpFil.surl = surl || '';
                                tmpFil.sopt = editoptions || {};
                                tmpFil.width = width;
                        ...
                        $.each(self.p.filterModel, function(i, n) {
                            for (var j = 0; j < gcolMod.length; j++) {
                                if (name == gcolMod[j].name) {
                                    index = gcolMod[j].index || name;
                                    break;
                                }
                            }
                            if (!index) {
                        ...
                        if (stype === 'select') {
                            ...
                            if (v) {
                                ...
                                if (self.p.marksearched) {
                                    $("#jqgh_" + name, gr.grid.hDiv).addClass("dirty-cell");
                                }
                                j++;
                            } else {
                                if (self.p.marksearched) {
                                    $("#jqgh_" + name, gr.grid.hDiv).removeClass("dirty-cell");
                                }
                                try {
                ...
                var formFill = function() {
                    ...
                    $.each(self.p.filterModel, function(i, n) {
                        ...
                        $(tl).append("<label for='" + name + "'>" + label + "</label>");
                        ...
                        case "select":
                            ...
                            break;
                        case 'text':
                            var df = defval ? defval : "";
                            $(td).append("<input type='text' name='" + (index || name) + "' id='sg_" + name + "' value='" + df + "'/>");
                            if ($t.sopt) {
                ...
                var frm = $("<form name='SearchForm' style=display:inline;' class='" + p.formclass + "'></form>");
                tbl = $("<table class='" + p.tableclass + "' cellspacing='0' cellpading='0' border='0'><tbody></tbody></table>");
                $(frm).append(tbl);
                formFill();
                $(this).append(frm);
