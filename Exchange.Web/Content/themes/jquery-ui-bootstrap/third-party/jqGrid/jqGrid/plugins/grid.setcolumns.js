                ...
                if ($("#" + IDs.themodal).html() != null) {
                    ...
                    for (i = 0; i < p.colNames.length; i++) {
                        if (!$t.p.colModel[i].hidedlg) { // added from T. Tomov
                            formdata += "<tr><td style='white-space: pre;'><input type='checkbox' style='margin-right:5px;' id='col_" + p.colModel[i].name + "' class='cbox' value='T' " +
                            ((p.colModel[i].hidden === false) ? "checked" : "") + "/>" + "<label for='col_" + p.colModel[i].name + "'>" + p.colNames[i] + ((p.colnameview) ? " (" + p.colModel[i].name + ")" : "") + "</label></td></tr>";
                        }
                    }
