    ...
    $.extend($.jgrid, {
        ...
        from: function(source) {
            ...
            var QueryObject = function(d, q) {
                ...
                _getOrder = function(data, by, dir, type, dfmt) {
                    var sortData = [],
                        _sortData = [],
                        newDir = dir == "a" ? 1 : -1,
                        i,
                ...
                _getGroup = function(data, by, dir, type, dfmt) {
                    var results = [],
                        group = null,
                        last = null,
                        val;
                ...
                andNot = function(f, v, x) {
                    _negate = !_negate;
                    return self.and(f, v, x);
                };
                orNot = function(f, v, x) {
                    _negate = !_negate;
                ...
                _compareValues = function(func, f, v, how, t) {
                    var fld;
                    if (_useProperties) {
                ...
                equals = function(f, v, t) {
                    return self._compareValues(self.equals, f, v, "==", t);
                };
                notEquals = function(f, v, t) {
                    return self._compareValues(self.equals, f, v, "!==", t);
                };
                isNull = function(f, v, t) {
                    return self._compareValues(self.equals, f, null, "===", t);
                };
                greater = function(f, v, t) {
                    return self._compareValues(self.greater, f, v, ">", t);
                };
                less = function(f, v, t) {
                    return self._compareValues(self.less, f, v, "<", t);
                };
                greaterOrEquals = function(f, v, t) {
                    return self._compareValues(self.greaterOrEquals, f, v, ">=", t);
                };
                lessOrEquals = function(f, v, t) {
                    return self._compareValues(self.lessOrEquals, f, v, "<=", t);
                };
                startsWith = function(f, v) {
                    var val = (v == null) ? f : v,
                ...
                groupBy = function(by, dir, type, datefmt) {
                    if (!self._hasData()) {
                        return null;
                    }
                    return self._getGroup(_data, by, dir, type, datefmt);
                };
                orderBy = function(by, dir, stype, dfmt) {
        ...
        getMethod: function(name) {
            return getAccessor($.fn.jqGrid, name);
        },
        ...
        return each(function() {
            ...
            var ts = this,
                grid = {
                    ...
                    dragStart: function(i, x, y) {
                        resizing = { idx: i, startX: x.clientX, sOL: y[0] };
                        hDiv.style.cursor = "col-resize";
                        curGbox = $("#rs_m" + $.jgrid.jqID(p.id), "#gbox_" + $.jgrid.jqID(p.id));
                        curGbox.css({ display: "block", left: y[0], top: y[1], height: y[2] });
                        ...
                        if (resizing) {
                            ...
                            if (newWidth > 33) {
                                ...
                                if (p.forceFit === true) {
                                    ...
                                    newWidth = p.direction === "ltr" ? p.tblwidth + diff : p.tblwidth - diff;
                                    h.newWidth = newWidth;
                                }
                            }
                        }
                    ...
                    dragEnd: function() {
                        ...
                        if (resizing) {
                            ...
                            if (p.forceFit === true) {
                                nw = headers[idx + p.nv].newWidth || headers[idx + p.nv].width;
                                headers[idx + p.nv].width = nw;
                                headers[idx + p.nv].el.style.width = nw + "px";
                                cols[idx + p.nv].style.width = nw + "px";
                                if (footers.length > 0) {
                                    footers[idx + p.nv].style.width = nw + "px";
                                }
                                p.colModel[idx + p.nv].width = nw;
                            } else {
                                p.tblwidth = newWidth || p.tblwidth;
                                ...
                                if (p.footerrow) {
                                    ...
                                    sDiv.scrollLeft = bDiv.scrollLeft;
                                }
                            }
            ...
            if (p.colNames.length === 0) {
                for (i = 0; i < p.colModel.length; i++) {
                    p.colNames[i] = p.colModel[i].label || p.colModel[i].name;
                }
            }
            ...
            $(eg).attr({ "id": "gbox_" + id, "dir": dir }).insertBefore(gv);
            $(gv).attr("id", "gview_" + id).appendTo(eg);
            $("<div class='ui-widget-overlay jqgrid-overlay' id='lui_" + id + "'></div>").insertBefore(gv);
            $("<div class='loading ui-state-default ui-state-active' id='load_" + id + "'>" + p.loadtext + "</div>").insertBefore(gv);
            $(this).attr({ cellspacing: "0", cellpadding: "0", border: "0", "role": "grid", "aria-multiselectable": !!p.multiselect, "aria-labelledby": "gbox_" + id });
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                emptyRows = function(scroll, locdata) {
                    ...
                    if (p.deepempty) {
                        ...
                        $(firstChild).empty().append(firstrow);
                    }
                    if (scroll && p.scroll) {
                        $(grid.bDiv.firstChild).css({ height: "auto" });
                        $(grid.bDiv.firstChild.firstChild).css({ height: 0, display: "none" });
                        if (grid.bDiv.scrollTop !== 0) {
                ...
                addXmlData = function(xml, t, rcnt, more, adjust) {
                    ...
                    if ($.isFunction(xmlRd.userdata)) {
                        ...
                        $.jgrid.getXmlData(xml, xmlRd.userdata, true).each(function() { ts.p.userData[getAttribute("name")] = $(this).text(); });
                    }
                ...
                addLocalData = function() {
                    ...
                    $.each(ts.p.colModel, function() {
                        ...
                        if (sorttype == "date" || sorttype == "datetime") {
                            if (formatter && typeof formatter === 'string' && formatter == 'date') {
                                if (formatoptions && formatoptions.srcformat) {
                                    srcformat = formatoptions.srcformat;
                                } else {
                                    srcformat = $.jgrid.formatter.date.srcformat;
                                }
                                if (formatoptions && formatoptions.newformat) {
                                    newformat = formatoptions.newformat;
                                } else {
                                    newformat = $.jgrid.formatter.date.newformat;
                                }
                            } else {
                                srcformat = newformat = datefmt || "Y-m-d";
                            }
                            cmtypes[name] = { "stype": sorttype, "srcfmt": srcformat, "newfmt": newformat };
                        } else {
                            cmtypes[name] = { "stype": sorttype, "srcfmt": '', "newfmt": '' };
                        }
                        if (ts.p.grouping) {
                            for (gin = 0, lengrp = grpview.groupField.length; gin < lengrp; gin++) {
                                if (name == grpview.groupField[gin]) {
                                    var grindex = name;
                                    if (index) {
                                        grindex = index;
                                    }
                        ...
                        if (!fndsort && (index == ts.p.sortname || name == ts.p.sortname)) {
                            st = name; // ???
                            fndsort = true;
                        }
                    });
                ...
                setPager = function(pgid, tp) {
                    ...
                    $('.ui-pg-selbox', "#" + pgcnt).bind('change', function() {
                        ...
                        ts.p.rowNum = value;
                        if (ts.p.pager) {
                            $('.ui-pg-selbox', ts.p.pager).val(value);
                        }
                        if (ts.p.toppager) {
                            $('.ui-pg-selbox', ts.p.toppager).val(value);
                        }
                        populate();
                        return false;
                    });
                ...
                setColWidth = function() {
                    ...
                    $.each(ts.p.colModel, function() {
                        ...
                        if (ts.p.grouping && ts.p.autowidth) {
                            var ind = $.inArray(name, ts.p.groupingView.groupField);
                            if (ind !== -1) {
                                hidden = !ts.p.groupingView.groupColumnShow[ind];
                            }
                        }
                        widthOrg = cw = intNum(width, 0);
                        if (hidden === false) {
                            initwidth += cw + brd;
                            if (fixed) {
                                gw += cw + brd;
                    ...
                    if (ts.p.shrinkToFit === true && vc > 0) {
                        ...
                        $.each(ts.p.colModel, function(i) {
                            if (hidden === false && !fixed) {
                                cw = Math.round(aw * width / (ts.p.tblwidth - brd * vc - gw));
                                width = cw;
                                initwidth += cw;
                                lvc = i;
                            }
            ...
            if (p.multiselect) {
                p.colNames.unshift("<input role='checkbox' id='cb_" + p.id + "' class='cbox' type='checkbox'/>");
                p.colModel.unshift({ name: 'cb', width: $.jgrid.cell_width ? ts.p.multiselectWidth + ts.p.cellLayout : ts.p.multiselectWidth, sortable: false, resizable: false, hidedlg: true, search: false, align: 'center', fixed: true });
            }
            if (p.rownumbers) {
                p.colNames.unshift("");
                p.colModel.unshift({ name: 'rn', width: ts.p.rownumWidth, sortable: false, resizable: false, hidedlg: true, search: false, align: 'center', fixed: true });
            }
                        ...
                        $(ts.rows).each(function(i) {
                            if (i > 0) {
                                if (!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled')) {
                                    $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + $.jgrid.jqID(id))[ts.p.useProp ? 'prop' : 'attr']("checked", true);
                                    $(this).addClass("ui-state-highlight").attr("aria-selected", "true");
                                    ts.p.selarrrow.push(id);
                                    ts.p.selrow = id;
                                    if (froz) {
                                        $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + $.jgrid.jqID(id), ts.grid.fbDiv)[ts.p.useProp ? 'prop' : 'attr']("checked", true);
                                        $("#" + $.jgrid.jqID(id), ts.grid.fbDiv).addClass("ui-state-highlight");
                                    }
                                }
                            }
                        });
                        ...
                        $(ts.rows).each(function(i) {
                            if (i > 0) {
                                if (!$(this).hasClass("ui-subgrid") && !$(this).hasClass('ui-state-disabled')) {
                                    $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + $.jgrid.jqID(id))[ts.p.useProp ? 'prop' : 'attr']("checked", false);
                                    $(this).removeClass("ui-state-highlight").attr("aria-selected", "false");
                                    emp.push(id);
                                    if (froz) {
                                        $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + $.jgrid.jqID(id), ts.grid.fbDiv)[ts.p.useProp ? 'prop' : 'attr']("checked", false);
                                        $("#" + $.jgrid.jqID(id), ts.grid.fbDiv).removeClass("ui-state-highlight");
                                    }
                                }
                            }
                        });
            ...
            firstr = null;
            var hTable = $("<table class='ui-jqgrid-htable' style='width:" + ts.p.tblwidth + "px' role='grid' aria-labelledby='gbox_" + id + "' cellspacing='0' cellpadding='0' border='0'></table>").append(thead),
            ...
            $(ts).before(grid.hDiv).click(function(e) {
                ...
                .bind('contextmenu', function(e) {
                    ...
                    if ($.isFunction(p.onRightClickRow)) {
            ...
            if (ts.p.toolbar[0]) {
                ...
                if (ts.p.toolbar[1] == "both") {
                    ...
                    $(grid.uDiv).addClass("ui-userdata ui-state-default").attr("id", "t_" + id).insertBefore(grid.hDiv);
                    $(grid.ubDiv).addClass("ui-userdata ui-state-default").attr("id", "tb_" + id).insertAfter(grid.hDiv);
                    if (hg) {
                        $(grid.ubDiv).hide();
                    }
                } else {
                    $(grid.uDiv).width(grid.width).addClass("ui-userdata ui-state-default").attr("id", "t_" + id);
                }
                if (hg) {
                    $(grid.uDiv).hide();
                }
    ...
    $.jgrid.extend({
                    ...
                    try {
                        $(p.colModel).each(function(i) {
                            ...
                            if (data[nm] !== undefined) {
                                lcdata[nm] = formatter && typeof formatter === 'string' && formatter == 'date' ? $.unformat.date.call(t, data[nm], this) : data[nm];
                                vl = t.formatter(rowid, data[nm], i, data, 'edit');
                                title = title ? { "title": $.jgrid.stripHtml(vl) } : {};
                                if (t.p.treeGrid === true && nm == t.p.ExpandColumn) {
        ...
        showHideCol: function(colname, show) {
            return each(function() {
                ...
                $(p.colModel).each(function(i) {
                    if ($.inArray(name, colname) !== -1 && hidden === sw) {
                        if ($t.p.frozenColumns === true && frozen === true) {
                            return true;
                        }
                        $("tr", $t.grid.hDiv).each(function() {
                            $(cells[i]).css("display", show);
                        });
                        ...
                        hidden = !sw;
                        fndh = true;
                        $($t).triggerHandler("jqGridShowHideCol", [sw, name, i]);
                    }
                });
                if (fndh === true) {
        ...
        hideCol: function(colname) {
            return each(function() { $(this).jqGrid("showHideCol", colname, "none"); });
        },
        showCol: function(colname) {
            return each(function() { $(this).jqGrid("showHideCol", colname, ""); });
        },
        ...
        setCaption: function(newcap) {
            return each(function() {
                p.caption = newcap;
                $("span.ui-jqgrid-title, span.ui-jqgrid-title-rtl", grid.cDiv).html(newcap);
                $(grid.cDiv).show();
            });
        },
    ...
    $.jgrid.extend({
        ...
        GridUnload: function() {
            return each(function() {
                ...
                if (p.pager) {
                    $(p.pager).empty().removeClass("ui-state-default ui-jqgrid-pager corner-bottom");
                }
                var newtable = document.createElement('table');
                $(newtable).attr({ id: defgrid.id });
                newtable.className = defgrid.cl;
                var gid = $.jgrid.jqID(id);
                $(newtable).removeClass("ui-jqgrid-btable");
                if ($(p.pager).parents("#gbox_" + gid).length === 1) {
                    $(newtable).insertBefore("#gbox_" + gid).show();
                    $(p.pager).insertBefore("#gbox_" + gid);
                } else {
                    $(newtable).insertBefore("#gbox_" + gid).show();
                }
                $(this).jqGrid('clearBeforeUnload');
        ...
        filterToolbar: function(p) {
            ...
            return each(function() {
                ...
                var triggerToolbar = function() {
                    ...
                    $.each($t.p.colModel, function() {
                        nm = index || name;
                        so = (searchoptions && searchoptions.sopt) ? searchoptions.sopt[0] : stype == 'select' ? 'eq' : p.defaultSearch;
                        v = $("#gs_" + $.jgrid.jqID(name), (frozen === true && $t.p.frozenColumns === true) ? $t.grid.fhDiv : $t.grid.hDiv).val();
                        if (v) {
                            sdata[nm] = v;
                            sopt[nm] = so;
                            j++;
                ...
                var clearToolbar = function(trigger) {
                    ...
                    $.each($t.p.colModel, function() {
                        var v;
                        if (searchoptions && searchoptions.defaultValue !== undefined) {
                            v = searchoptions.defaultValue;
                        }
                        nm = index || name;
                        switch (stype) {
                        case 'select':
                            $("#gs_" + $.jgrid.jqID(name) + " option", (frozen === true && $t.p.frozenColumns === true) ? $t.grid.fhDiv : $t.grid.hDiv).each(function(i) {
                                if (i === 0) {
                                    selected = true;
                                }
                                if ($(this).val() == v) {
                            ...
                            break;
                        case 'text':
                            $("#gs_" + $.jgrid.jqID(name), (frozen === true && $t.p.frozenColumns === true) ? $t.grid.fhDiv : $t.grid.hDiv).val(v);
                            if (v !== undefined) {
                                sdata[nm] = v;
                                j++;
                            } else {
                ...
                $.each($t.p.colModel, function() {
                    ...
                    soptions = $.extend({}, searchoptions || {});
                    if (search) {
                        switch (stype) {
                        case "select":
                            surl = surl || soptions.dataUrl;
                            if (surl) {
                ...
                if (o.useColSpanStyle) {
                    ...
                    $htable.find("span.ui-jqgrid-resize").each(function() {
                        ...
                        if ($parent.is(":visible")) {
                            style.cssText = 'height: ' + $parent.height() + 'px !important; cursor: col-resize;';
                        }
                    });
                    ...
                    $($t).bind('jqGridAfterGridComplete.setFrozenColumns', function() {
                        ...
                        if ($t.p.hoverrows === true) {
                            $("tr.jqgrow", btbl).hover(
                                function() {
                                    $(this).addClass("ui-state-hover");
                                    $("#" + $.jgrid.jqID(id), "#" + $.jgrid.jqID($t.p.id)).addClass("ui-state-hover");
                                },
                                function() {
                                    $(this).removeClass("ui-state-hover");
                                    $("#" + $.jgrid.jqID(id), "#" + $.jgrid.jqID($t.p.id)).removeClass("ui-state-hover");
                                }
                            );
                            $("tr.jqgrow", "#" + $.jgrid.jqID($t.p.id)).hover(
                                function() {
                                    $(this).addClass("ui-state-hover");
                                    $("#" + $.jgrid.jqID(id), "#" + $.jgrid.jqID($t.p.id) + "_frozen").addClass("ui-state-hover");
                                },
                                function() {
                                    $(this).removeClass("ui-state-hover");
                                    $("#" + $.jgrid.jqID(id), "#" + $.jgrid.jqID($t.p.id) + "_frozen").removeClass("ui-state-hover");
                                }
                            );
                        }
                        btbl = null;
...
(function($) {
    $.fn.jqm = function(o) {
        ...
        return each(function() {
            if (_jqm)return H[_jqm].c = $.extend({}, H[_jqm].c, o);
            s++;
            _jqm = s;
    ...
    $.fn.jqmShow = function(t) { return each(function() { $.jqm.open(_jqm, t); }); };
    $.fn.jqmHide = function(t) { return each(function() { $.jqm.close(_jqm, t); }); };
...
var xmlJsonClass = {
    ...
    xml2json: function(xml, tab) {
        ...
        var nws = removeWhite(xml);
        var obj = toObj(nws);
        var json = toJson(obj, xml.nodeName, "\t");
    ...
    toJson: function(o, name, ind, wellform) {
        ...
        if (o === "[]") {
            ...
            for (i = 0, n = o.length; i < n; i += 1) {
                ar[i] = toJson(o[i], "", ind + tab, wellform);
            }
            ...
            for (m in o) {
                if (o.hasOwnProperty(m)) {
                    arr[arr.length] = toJson(o[m], m, ind + tab, wellform);
                }
            }
        ...
        info_dialog: function(caption, content, c_b, modalopt) {
            ...
            if (buttstr) {
                $.each(mopt.buttons, function(i) {
                    $("#" + $.jgrid.jqID(id), "#info_id").bind('click', function() {
...
(function($) {
    ...
    $.fn.jqFilter = function(arg) {
        ...
        return each(function() {
            ...
            var i,
                len = p.columns.length,
                cl,
                isIE = /msie/i.test(navigator.userAgent) && !window.opera;

            // translating the options
            if (p._gridsopt.length) {
                // ['eq','ne','lt','le','gt','ge','bw','bn','in','ni','ew','en','cn','nc']
                for (i = 0; i < p._gridsopt.length; i++) {
                    p.ops[i].description = p._gridsopt[i];
                }
            }
		 ...
		 */
            createTableRowForRule = function(rule, group) {
                ...
                ruleFieldSelect.bind('change', function() {
                    ...
                    var s = "", so = 0;
                    aoprs = [];
                    $.each(that.p.ops, function() { aoprs.push(name); });
    ...
    $.jgrid.extend({
            ...
            return each(function() {
                ...
                if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
                    ...
                    $("#" + fid).jqFilter({
                        ...
                        onChange: function() {
                            if (p.showQuery) {
                                $('.query', this).html(toUserFriendlyString());
                            }
            ...
            return each(function() {
                ...
                function getFormData() {
                    $(frmtb + " > tbody > tr > td > .FormElement").each(function() {
                        ...
                        if (celm.length) {
                            ...
                            $.each($t.p.colModel, function() {
                                if (name === nm && editoptions && $.isFunction(editoptions.custom_value)) {
                                    try {
                                        postdata[nm] = editoptions.custom_value.call($t, $("#" + $.jgrid.jqID(nm), frmtb), 'get');
                            ...
                            case "checkbox":
                                ...
                                break;
                            case "select-one":
                                postdata[name] = $("option:selected", this).val();
                                extpost[name] = $("option:selected", this).text();
                                break;
                            case "select-multiple":
                                postdata[name] = $(this).val();
                                if (postdata[name]) {
                                    postdata[name] = postdata[name].join(",");
                                } else {
                                    postdata[name] = "";
                                }
                                var selectedText = [];
                                ...
                                extpost[name] = selectedText.join(",");
                                break;
                            case "password":
                            case "text":
                            case "textarea":
                            ...
                            if ($t.p.autoencode) {
                                postdata[name] = $.jgrid.htmlEncode(postdata[name]);
                            }
                        }
                    });
                    return true;
                ...
                function createData(rowid, obj, tb, maxcols) {
                    ...
                    $(obj.p.colModel).each(function(i) {
                        ...
                        if (editrules && editrules.edithidden === true) {
                            hc = false;
                        } else {
                            hc = hidden === true ? true : false;
                        }
                        ...
                        if (nm !== 'cb' && nm !== 'subgrid' && editable === true && nm !== 'rn') {
                            if (ind === false) {
                                ...
                                if (nm == obj.p.ExpandColumn && obj.p.treeGrid === true) {
                                    ...
                                    try {
                                        ...
                                        tmp = (edittype && edittype == "textarea") ? $("td[role='gridcell']:eq(" + i + ")", obj.rows[ind]).text() : $("td[role='gridcell']:eq(" + i + ")", obj.rows[ind]).html();
                                    }
                            ...
                            var opt = $.extend({}, editoptions || {}, { id: nm, name: nm }),
                                frmopt = $.extend({}, { elmprefix: '', elmsuffix: '', rowabove: false, rowcontent: '' }, formoptions || {}),
                            ...
                            elc = $.jgrid.createEl.call($t, edittype, opt, tmp, false, $.extend({}, $.jgrid.ajaxOptions, obj.p.ajaxSelectOptions || {}));
                            if (tmp === "" && edittype == "checkbox") {
                                tmp = $(elc).attr("offval");
                            }
                            if (tmp === "" && edittype == "select") {
                        ...
                        var ajaxOptions = $.extend({
                            ...
                            complete: function(data, status) {
                                ...
                                if (ret[0] === false) {
                                    ...
                                    $.each($t.p.colModel, function() {
                                        if (extpost[name] && formatter && formatter == 'select') {
                                            try {
                                                delete extpost[name];
                                            } catch (e) {
                                            }
            ...
            return each(function() {
                ...
                function createData(rowid, obj, tb, maxcols) {
                    ...
                    $(obj.p.colModel).each(function() {
                        ...
                        if (!hc && align === 'right') {
                            if (formatter && $.inArray(formatter, fmtnum) !== -1) {
                                max1 = Math.max(max1, parseInt(width, 10));
                            } else {
                                max2 = Math.max(max2, parseInt(width, 10));
                            }
                        }
                    });
                    ...
                    $(obj.p.colModel).each(function(i) {
                        ...
                        dc = hc ? "style='display:none'" : "";
                        viewfld = (typeof viewable !== 'boolean') ? true : viewable;
                        if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && viewfld) {
                            ...
                            setme = align === 'right' && maxw !== 0 ? true : false;
                            var frmopt = $.extend({}, { rowabove: false, rowcontent: '' }, formoptions || {}),
                                rp = parseInt(frmopt.rowpos, 10) || cnt + 1,
                                cp = parseInt((parseInt(frmopt.colpos, 10) || 1) * 2, 10);
                            if (frmopt.rowabove) {
                                var newdata = $("<tr><td class='contentinfo' colspan='" + (maxcols * 2) + "'>" + frmopt.rowcontent + "</td></tr>");
        ...
        navGrid: function(elem, o, pEdit, pAdd, pDel, pSearch, pView) {
            ...
            return each(function() {
                ...
                var alertIDs = { themodal: 'alertmod_' + p.id, modalhead: 'alerthd_' + p.id, modalcontent: 'alertcnt_' + p.id },
                    $t = this,
                    twd,
                    tdw;
        ...
        restoreRow: function(rowid, afterrestorefunc) {
            ...
            return each(function() {
                ...
                if (fr >= 0) {
                    ...
                    $.each($t.p.colModel, function() {
                        if (editable === true && $t.p.savedRow[fr].hasOwnProperty(name)) {
                            ares[name] = $t.p.savedRow[fr][name];
                        }
                    });
        ...
        addRow: function(p) {
            ...
            return each(function() {
                ...
                if (p.useDefValues === true) {
                    $($t.p.colModel).each(function() {
                        if (editoptions && editoptions.defaultValue) {
                            var opt = editoptions.defaultValue,
    ...
    $.jgrid.extend({
        ...
        addSubGrid: function(pos, sind) {
            return each(function() {
                ...
                $.each(ts.p.colModel, function() {
                    if (hidden === true || name === 'rn' || name === 'cb') {
                        nhc++;
                    }
                });
        ...
        expandNode: function(rc) {
            return each(function() {
                ...
                if (!rc[expanded]) {
                    ...
                    var rc1 = $("#" + $.jgrid.jqID(id), grid.bDiv)[0];
                    ...
                    if (v !== undefined) {
                        if (irow === 0) {
                            ...
                            $.each(grp.counters[i].summary, function() {
                                if ($.isFunction(st)) {
                                    v = st.call($t, v, nm, record);
                                } else {
                                    v = $($t).jqGrid('groupingCalculations.handler', st, v, nm, sr, srt, record);
                                }
                            });
                            ...
                            for (k = 0; k < colspans; k++) {
                                ...
                                $.each(fdata.summary, function() {
                                    if (nm === cm[k].name) {
                                        ...
                                        if (typeof st === 'string' && st.toLowerCase() === 'avg') {
                                            if (v && grlen > 0) {
                                                v = (v / grlen);
                                            }
                                        }
                                        try {
                                            vv = $t.formatter('', v, k, this);
                                        } catch (ef) {
                                            vv = v;
                                        }
    ...
    $.jgrid.extend({
                ...
                var sortable_opts = {
                    ...
                    "update": function(event, ui) {
                        var p = $(ui.item).parent(),
                            ...
                            cmMap = {},
                            tid = ts.p.id + "_";
                        $.each(colModel, function(i) { cmMap[name] = i; });
                        var permutation = [];
                        th.each(function() {
        ...
        columnChooser: function(opts) {
               ...
               "done" function */
                "apply_perm": function() {
                    $('option', select).each(function() {
                        if (selected) {
                            self.jqGrid("showCol", colModel[value].name);
                        } else {
                            self.jqGrid("hideCol", colModel[value].name);
                        }
                    });

                    var perm = [];
                    //fixedCols.slice(0);
                    $('option:selected', select).each(function() { perm.push(parseInt(value, 10)); });
            ...
            $.each(colModel, function(i) {
                ...
                (hidden ? "" : "selected='selected'") + ">" + $.jgrid.stripHtml(colNames[i]) + "</option>");
            });
                ...
                if ($.fn.sortable) {
                    ...
                    opts.start = function(ev, ui) {
                        ...
                        $("td", ui.item).each(function(i) {
                            style.width = $t.grid.cols[i].style.width;
                        });
                        if ($t.p.subGrid) {
                        ...
                        return $.extend({
                            start: function(ev, ui) {
                                ...
                                $("td", ui.helper).each(function(i) {
                                    style.width = $t.grid.headers[i].width + "px";
                                });
                        ...
                        return $.extend({
                            accept: function(d) {
                                ...
                                if (tid.length > 0 && $.data(tid[0], "dnd") !== undefined) {
                                    ...
                                    return $.inArray('#' + $.jgrid.jqID(id), cn) != -1 ? true : false;
                                }
                                return false;
                            },
                            drop: function(ev, ui) {
                                ...
                                if (!opts.dropbyname) {
                                    ...
                                    var dropmodel = $("#" + $.jgrid.jqID(id)).jqGrid('getGridParam', 'colModel');
                                    try {
                                        for (key in getdata) {
                                ...
                                if (ui.helper.dropped) {
                                    ...
                                    $("#" + $.jgrid.jqID(id)).jqGrid('addRowData', grid, getdata, opts.droppos);
                                }
